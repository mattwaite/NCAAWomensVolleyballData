...
```{r}
urls <- read_csv("url_csvs/ncaa_womens_volleyball_teamurls_2018.csv") %>% pull(3)
```
...
```{r}
urls <- c("https://stats.ncaa.org/player/game_by_game?game_sport_year_ctl_id=15820&org_id=26172&stats_player_seq=-100")
...
```{r}
matches %>% rename(opponent_info = opponent) %>% mutate(opponent = case_when(opponent_info == "Defensive Totals" ~ lag(opponent_info, n=1), TRUE ~ opponent_info))
```
